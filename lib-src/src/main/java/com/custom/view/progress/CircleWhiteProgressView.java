package com.custom.view.progress;
 ...
 * Description: This is CircleWhiteProgressView
 */
public class CircleWhiteProgressView extends View {
    ...
    public CircleWhiteProgressView(Context context) {
        this(context, null);
    }

    public CircleWhiteProgressView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircleWhiteProgressView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        @SuppressLint("Recycle")
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CircleWhiteProgressView);
        ...
        mBackPaint.setStrokeWidth(typedArray.getDimension(R.styleable.CircleWhiteProgressView_backWidth, 5));
        mBackPaint.setColor(typedArray.getColor(R.styleable.CircleWhiteProgressView_backColor, Color.LTGRAY));
        ...
        mProgPaint.setStrokeWidth(typedArray.getDimension(R.styleable.CircleWhiteProgressView_progWidth, 10));
        mProgPaint.setColor(typedArray.getColor(R.styleable.CircleWhiteProgressView_progColor, Color.BLUE));

        // 初始化进度圆环渐变色
        int startColor = typedArray.getColor(R.styleable.CircleWhiteProgressView_progStartColor, -1);
        int firstColor = typedArray.getColor(R.styleable.CircleWhiteProgressView_progFirstColor, -1);
        if (startColor != -1 && firstColor != -1) mColorArray = new int[]{startColor, firstColor};
        else mColorArray = null;

        // 初始化进度
        mProgress = typedArray.getInteger(R.styleable.CircleWhiteProgressView_progress, 0);
        typedArray.recycle();
    }
