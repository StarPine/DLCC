...
import com.faceunity.nama.utils.BeautyConfigManagerUtil;
...
public class FaceBeautyControlView extends BaseControlView {
    ...
    public void bindDataFactory(AbstractFaceBeautyDataFactory dataFactory) {
        ...
        //加载本地滤镜数据
        List<FaceBeautyFilterBean> faceBeautyFilters = BeautyConfigManagerUtil.getInstance().getFaceBeautyFilters();
        if(faceBeautyFilters != null){
            mFilters.clear();
            mFilters.addAll(faceBeautyFilters);
        }
        mFiltersAdapter.setData(mFilters);

        mDataFactory.setCurrentFilterIndex(BeautyConfigManagerUtil.getInstance().getFaceBeautyFiltersIndex());

        //加载本地美颜数据
        List<FaceBeautyLocalBean> faceBeautyParam = BeautyConfigManagerUtil.getInstance().getFaceBeautyParam();
        if (faceBeautyParam != null) {
            for (FaceBeautyLocalBean bean : faceBeautyParam) {
                mDataFactory.updateParamIntensity(bean.getKey(), bean.getValue());
            }
        }
            ...
            public void onProgressChanged(DiscreteSeekBar seekBar, int value, boolean fromUser) {
                ...
                if (checkGroup.getCheckedCheckBoxId() == R.id.beauty_radio_skin_beauty) {
                    ...
                    if (!DecimalUtils.doubleEquals(bean.getIntensity(), valueF)) {
                        ...
                        mFilters.set(mDataFactory.getCurrentFilterIndex(),bean);
...
//        保存美颜相关数值到本地
        HashMap<String, FaceBeautyDataFactory.FaceBeautyGetParamInterface> faceBeautyParam = mDataFactory.getFaceBeautyParam();
        List<FaceBeautyLocalBean> faceBeautyLocalBeans = new ArrayList<>();
        for (String key : faceBeautyParam.keySet()) {
            FaceBeautyLocalBean bean = new FaceBeautyLocalBean(key, faceBeautyParam.get(key).getValue());
            faceBeautyLocalBeans.add(bean);
        }
        BeautyConfigManagerUtil.getInstance().putFaceBeautyParam(faceBeautyLocalBeans);

        //保存滤镜数据
        BeautyConfigManagerUtil.getInstance().putFaceBeautyFilters(mFilters);
        BeautyConfigManagerUtil.getInstance().putFaceBeautyFiltersIndex(mDataFactory.getCurrentFilterIndex());
