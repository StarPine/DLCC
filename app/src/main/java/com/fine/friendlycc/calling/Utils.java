...
public class Utils {
    ...
    public static void startCallSomeone(int type, String toUserId, int roomId, String data) {
        if (type == TRTCCalling.TYPE_VIDEO_CALL) {
            ...
            TUICallingImpl.sharedInstance(CCApplication.instance()).call(userIDs, TUICalling.Type.VIDEO, roomId, data);
        } else if (type == TRTCCalling.TYPE_AUDIO_CALL) {
            String[] userIDs = {toUserId};
            TUICallingImpl.sharedInstance(CCApplication.instance()).call(userIDs, TUICalling.Type.AUDIO, roomId, data);
        }
    }
